...
public class BorrowBookResource {
    ...
    public ResponseEntity<BorrowBook> createBorrowBook(@RequestParam("borrowBook") String borrowBook,
        ...
        try {
            ...
            if (result != null) {
                return ResponseEntity.created(new URI("/api/borrow-books/" + result.getId())).headers(HeaderUtil
        ...
        try {
            ...
            if (result != null) {
                return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(applicationName, true,
                        ENTITY_NAME, borrowBookData.getId().toString())).body(result);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    @GetMapping("/borrow-books/search")
    public ResponseEntity<List<BorrowBook>> findByDateUpdate(Pageable pageable, String dateUpdate) {
        log.debug("REST request to get a page of BorrowBooks");
        Page<BorrowBook> page = borrowBookService.findByDateUpdate(pageable, dateUpdate);
        HttpHeaders headers = PaginationUtil
                .generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
