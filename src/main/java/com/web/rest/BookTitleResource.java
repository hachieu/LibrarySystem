...
public class BookTitleResource {
    ...
    /**
     * {@code GET  /book-titles} : get all the bookTitles.
     *
     * @param pageable the pagination information.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list
     *         of bookTitles in body.
     */
    @GetMapping("/book-titles")
    public ResponseEntity<List<BookTitle>> getAllBookTitles(Pageable pageable) {
        log.debug("REST request to get a page of BookTitles");
        Page<BookTitle> page = bookTitleService.findAll(pageable);

        // httpServletResponse.setHeader("urlImage", folderName);
